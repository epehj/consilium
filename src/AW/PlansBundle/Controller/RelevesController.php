...
class RelevesController extends Controller
{
  ...
  public function addPoseurAction(Request $request, Commande $commande) {

      $form = $this->get('form.factory')->create(AddPoseurType::class, $commande);
      $form->handleRequest($request);
        if($request->isMethod('POST') and $form->isValid()) {
            $this->getDoctrine()->getManager()->flush();
            return $this->redirectToRoute('aw_plans_pose_end', array('id' => $commande->getId()));
        }
      return $this->render('AWPlansBundle:Releves:addPoseur.html.twig',
          array(
              'commande' => $commande,
              'form' => $form->createView()
          ));
    ...
    public function terminerReleveAction(Request $request, Commande $commande) {
      ...
      //RAZ des ano s'il en existait d'anciennes (demande de Quentin)
        ...
        if($request->isMethod('POST') and $form->isValid()) {
            ...
            $this->getDoctrine()->getManager()->flush();
            // si ano, on repasse en view sans rien changer
            if($commandeST->getAnomalies()->count() > 0)
                return $this->redirectToRoute('aw_plans_releves_view', array('id'=>$commande->getId()));
            else if($commandeST->isCancel()){
